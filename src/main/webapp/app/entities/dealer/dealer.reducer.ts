...
export const ACTION_TYPES = {
  SEARCH_DEALERS: 'dealer/SEARCH_DEALERS',
...
export default (state: DealerState = initialState, action): DealerState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_DEALERS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_DEALERS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_DEALERS):
...
export const getSearchEntities: ICrudSearchAction<IDealer> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_DEALERS,
  payload: axios.get<IDealer>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
